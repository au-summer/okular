...
QString LlmClient::buildPrompt(const QList<ParagraphData> &paragraphs) const
{
    QString prompt = QStringLiteral(
        "You are an academic paper analysis assistant. Analyze the following paragraphs from a PDF page.\n"
...
void LlmClient::requestPageSummary(int pageIdx, const QList<ParagraphData> &paragraphs)
{
    ...
    userMsg[QStringLiteral("content")] = buildPrompt(paragraphs);
...
QList<ParagraphLlmResult> LlmClient::parseResponse(const QByteArray &data) const
{
    ...
    // OpenAI response: choices[0].message.content contains the JSON string
    ...
    QString content = choices[0].toObject()[QStringLiteral("message")].toObject()[QStringLiteral("content")].toString();
